# Windsurf Cascade Instructions for SDD Projects

Paste these instructions into Windsurf's Cascade system prompt or `.windsurfrules` file.
...
> **Windsurf Note:** Windsurf's Cascade can execute multi-step flows automatically. Use Cascade flows to chain ticket execution steps (read backlog -> implement -> test -> update docs -> update backlog). This makes SDD's structured workflow a natural fit for Cascade's sequential reasoning.
...
Use a single Cascade flow to execute these steps in sequence:
...
Cascade will handle each step sequentially, reading context and applying changes in order.
...
Windsurf's Cascade supports multi-step flows. When working on a sprint with multiple tickets:

### Cascade Flow: Backend Tickets
- Focus: Database, API, Services
- Tickets: Owner = "Backend" or "DB"
- DO NOT modify frontend code

### Cascade Flow: Frontend Tickets
- Focus: Components, Pages, State
- Tickets: Owner = "Frontend"
- DO NOT modify backend code
- Check backlog before starting tickets that depend on backend work

Chain multiple ticket executions in a single Cascade session when tickets have no dependencies.
