...
|-------|---------|-------------|
| **GPT-5.2** | Migrations, configs, CRUD, boilerplate, docs, repetitive tests | 40% |
| **GPT-5.3** | Services, components, integrations, validation, forms | 45% |
| **GPT-5.4** | Planning, architecture, security, complex debugging, QA synthesis | 15% |

The `Model` column in `specs/backlog.md` recommends the model for each ticket.

### Copilot Routing Policy

- Use `gpt-5.4` to plan sprints, break down ambiguous work, review cross-cutting changes, and investigate hard failures
- Use `gpt-5.3` for normal ticket execution once the plan is clear
- Use `gpt-5.2` for low-risk, pattern-based edits such as scaffolding, fixtures, docs, and straightforward tests
- Reuse planning artifacts from `gpt-5.4` in lower-cost execution chats instead of re-planning from scratch
