...
| `vscode-copilot` | VS Code with GitHub Copilot | GPT-5.4 planner + GPT-5.3/5.2 executors | IDE-integrated, multi-chat orchestration |
...
- Planning vs execution model routing rules and escalation criteria
...
│   ├── finops.md                       # Model selection optimizer (GPT-5.x routing)
...
- Model column: for `vscode-copilot`, use `gpt-5.2` (40%), `gpt-5.3` (45%), `gpt-5.4` (15%). Use equivalent environment-specific tiers for other environments.
...
|--------|-------------------|-----------|
[Generate based on ticket content - for `vscode-copilot`: `gpt-5.2` for simple/boilerplate/docs/tests, `gpt-5.3` for standard implementation, `gpt-5.4` for planning/architecture/security/debugging]
...
#### Ticket X.Y: [Title]
**Owner:** [Role]  |  **Model:** [gpt-5.2/gpt-5.3/gpt-5.4]  |  **Points:** [N]
...
- **GPT-5.2 (40%):** Simple, repetitive execution work
- **GPT-5.3 (45%):** Standard feature implementation
- **GPT-5.4 (15%):** Planning, architecture, security, and complex debugging
...
- Simple CRUD endpoints with minimal business logic
- Documentation updates and repetitive cleanup
- Basic component shells and low-risk refactors
- Straightforward unit tests and fixture generation

### Use GPT-5.3 For:
- Services with business logic
- React components with state and routing
- API endpoints with validation and integrations
- Integration tests and standard QA fixes
- Auth flows, forms, and shared abstractions
- Most ticket execution work after planning is complete

### Use GPT-5.4 For:
- Sprint planning and backlog shaping
- Architecture decisions and system trade-offs
- Security-critical code and policy logic
- Complex debugging and root-cause analysis
- Cross-cutting QA review and synthesis
- Prompt strategy and model-routing design

## Copilot Routing Rules

- Default to `gpt-5.3` for normal implementation tickets
- Start with `gpt-5.2` for boilerplate, tests, docs, and obvious pattern-following work
- Escalate to `gpt-5.4` when the task involves architecture, uncertainty, security, novel logic, or repeated failure
- Planning artifacts produced by `gpt-5.4` should be reused by `gpt-5.2` and `gpt-5.3` instead of re-planning from scratch

## Cost Estimation

| Model | Relative Cost | Typical Sprint Usage |
|-------|---------------|---------------------|
| GPT-5.2 | Lowest | 40% of tokens |
| GPT-5.3 | Medium | 45% of tokens |
| GPT-5.4 | Highest | 15% of tokens |

## Sprint Cost Forecast

For a 12-ticket sprint:
- 4 tickets × GPT-5.2 = baseline cost
- 6 tickets × GPT-5.3 = standard execution cost
- 2 tickets × GPT-5.4 = premium planning/review cost
- **Total:** optimize for quality while avoiding all-`gpt-5.4` usage

**Savings vs all-GPT-5.4:** target 40-60% depending on sprint shape
...
Dashboard ("/"):
  ...
  - **Recharts PieChart**: Model distribution (`gpt-5.2`=green, `gpt-5.3`=blue, `gpt-5.4`=amber)
...
Backlog ("/backlog"):
  ...
  **Tab 1: Kanban Board** (default view, drag-disabled display only):
    ...
    - shadcn/ui Cards per ticket: title, owner Avatar/Badge, model Badge
      (`gpt-5.2`=green, `gpt-5.3`=blue, `gpt-5.4`=amber), points, dependency indicator
  ...
  **Tab 2: Sprint Table** (per-sprint view):
    ...
    - shadcn/ui Table with sortable columns:
      ...
      - Model Badge (`gpt-5.2`=green, `gpt-5.3`=blue, `gpt-5.4`=amber)
      - Points column
...
│                                 #     modelDistribution: { gpt_5_2: N, gpt_5_3: N, gpt_5_4: N },
