export const slidesEN = [
  ...
  // 12. agentic5Acts — SCROLLABLE
  {
    ...
    closingCallout: "The team that governs before coding ships faster than the team that codes before thinking. Every Bolt closes with memory — and every memory makes the next Bolt faster.",
    acts: [
      {
        number: 'I',
        name: 'SPECIFY',
        ...
        accent: 'blue',
        bullets: [
          'spec.md defines the problem only — not implementation. GenAI turns unstructured inputs into clear, approved requirements.',
          'constitution.md locks the tech stack, security protocols, and architecture standards — the laws agents cannot break.',
          'No agent may begin implementation until the Product Owner formally approves the spec.',
          'Anti-spec-rot: spec.md is version-controlled, updated before code changes. Kills context drift at the source.',
        ],
      ...
      },
      {
        ...
        stageRef: 'Stage 3',
        owners: ['Architect', 'Dev Team Lead'],
        hero: false,
        accent: 'emerald',
        bullets: [
          'Agents produce plan.md (technical strategy) and tasks.md (granular, independently-testable units).',
          'Senior engineers interrogate the plan — not write it — focusing on legacy constraints, security, blast radius.',
          'Team Lead approves the implementation map before any code runs.',
          'Each task independently testable — this is what makes Bolts possible: hours, not weeks.',
        ],
      ...
      },
      {
        ...
        stageRef: 'Stages 4–5',
        owners: ['Developer (HITL)', 'QA Developer (HITL)'],
        hero: true,
        accent: 'indigo',
        bullets: [
          'TDD-enforced: no implementation before a failing test exists. Agents run in secure, isolated worktrees.',
          'Every agent error is encoded as a permanent harness test — it cannot recur.',
          'Developer = Quality Orchestrator: reviews agent PRs, maintains the harness, guards edge cases.',
          "QA agents build E2E suites aligned to the spec's acceptance criteria. AI review drives 38.7% of real fixes.",
          'Parallel execution: independent tasks run simultaneously in isolated worktrees.',
        ],
        output: 'Merged, tested, verified code',
      },
      {
        number: 'IV',
        name: 'REMEMBER',
        ...
        stageRef: 'Stage 6',
        owners: ['Product Owner (HITL)', 'Dev Team Lead (HITL)', 'QA Lead (HITL)'],
        hero: true,
        accent: 'teal',
        bullets: [
          ...
          'The next agent reads the summary and inherits full context in 30 seconds. Zero reverse engineering.',
      ...
      },
      {
        ...
        accent: 'orange',
        bullets: [
          'Observability agents monitor real telemetry against spec intent; drift triggers alerts or a self-correction Bolt.',
          'SRE agents proactively open issues on anomalies; GenAI writes incident summaries.',
          'CI/CD discipline carries into the agentic world — all agent communications logged for audit.',
          'The loop never ends: STEER detects drift → triggers a new Bolt → STEER resumes.',
        ],
        output: 'A self-steering, spec-aligned system',
      },
    ],
  },
