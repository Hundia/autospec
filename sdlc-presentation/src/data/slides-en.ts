export const slidesEN = [
  ...
  // 6. secondFeature (merged: The Reverse Engineering Tax + The Breaking Point)
  {
    ...
    chapter1: {
      ...
      headline: 'Day 60. The drone flies. You want Feature +1.',
      setup: [
        'Game Of Drones runs in production. NAV and STATUS telemetry — live, zero incidents.',
        'The chat sessions that built it are gone. No ICD was ever documented. No spec was ever written.',
        'You open a fresh session: "Add the video stream." Should be simple.',
      ],
      terminal: {
        windowTitle: 'ai-agent — session #51 — day 60',
        lines: [
          { kind: 'you', time: '09:02', text: 'Add a live video streaming interface. The FPV camera module already exists on the drone.' },
          { kind: 'agent', time: '09:02', text: 'I have no context for this project. 180+ files. No ICD changelog. No spec. Three different message framing patterns in the codebase.' },
          { kind: 'agent', time: '09:03', text: 'Which MFL state machine version is active? Is MFL-CAM-002 the canonical camera frame? Who owns the ICD?' },
          ...
          { kind: 'cmd', text: "grep -r 'MFL' src/", result: '→ 89 matches across 14 files' },
          { kind: 'cmd', text: 'cat src/services/icd.ts', result: '→ 847 lines, no comments' },
          { kind: 'cmd', text: "find . -name 'spec*.md'", result: '→ 0 results' },
          { kind: 'sys', time: '11:30', text: '2.5 hrs elapsed · feature code written: 0', tone: 'warn' },
          { kind: 'agent', time: '14:15', text: 'Found 3 conflicting ICD implementations — ICDv1, ICDv2, and ICDv2-patch. Which is authoritative?' },
