export const slidesHE = [
  ...
  // 7. secondFeature (merged: The Reverse Engineering Tax + The Breaking Point)
  {
    ...
    chapter1: {
      ...
      headline: 'יום 60. הרחפן עף. אתה רוצה פיצ\'ר +1.',
      setup: [
        'Game Of Drones רץ בפרודקשן. טלמטריית NAV ו-STATUS — בשידור חי, אפס תקלות.',
        'סשני הצ\'אט שבנו אותו נעלמו. ה-ICD מעולם לא תועד. ספסיפיקציה מעולם לא נכתבה.',
        'אתה פותח סשן חדש: "הוסף שידור וידאו." אמור להיות פשוט.',
      ],
      terminal: {
        windowTitle: 'ai-agent — session #51 — day 60',
        lines: [
          { kind: 'you', time: '09:02', text: 'הוסף ממשק שידור וידאו חי. מודול המצלמה FPV כבר קיים על הרחפן.' },
          { kind: 'agent', time: '09:02', text: 'אין לי שום הקשר לפרויקט הזה. 180+ קבצים. אין לוג שינויים ל-ICD. אין ספסיפיקציה. שלוש תבניות פריימינג שונות להודעות בקוד.' },
          { kind: 'agent', time: '09:03', text: 'איזו גרסת state machine של MFL פעילה? האם MFL-CAM-002 הוא פריים המצלמה הקנוני? מי מחזיק ב-ICD?' },
          { kind: 'you', time: '09:04', text: 'אני לא זוכר. הכל הוחלט בצ\'אט. הסשנים נעלמו.' },
          { kind: 'agent', time: '09:05', text: 'הבנתי. מתחיל גילוי.' },
          { kind: 'cmd', text: "grep -r 'MFL' src/", result: '→ 89 התאמות ב-14 קבצים' },
          { kind: 'cmd', text: 'cat src/services/icd.ts', result: '→ 847 שורות, אפס הערות' },
          { kind: 'cmd', text: "find . -name 'spec*.md'", result: '→ 0 תוצאות' },
          { kind: 'sys', time: '11:30', text: '2.5 שעות חלפו · קוד פיצ\'ר שנכתב: 0', tone: 'warn' },
          { kind: 'agent', time: '14:15', text: 'מצאתי 3 מימושי ICD סותרים — ICDv1, ICDv2, ו-ICDv2-patch. איזה מהם סמכותי?' },
  ...
  // 17. tooling — SCROLLABLE · Make It Real (simplified)
  {
    ...
    specPage: {
      ...
      frameworks: [
        { id: 'spec-kit', glyph: '⚙️', name: 'Spec-Kit', short: 'Spec-Kit', role: 'Specify → Plan → Tasks → Implement', accent: 'teal', tagline: 'הספסיפיקציה במרכז; אין קוד עד שהספסיפיקציה מאושרת.', whatItIs: 'המסגרת הפתוחה של GitHub שמובילה כל שינוי דרך שלבים מפורשים. constitution קובע את כללי הבית, ‎/specify‎ לוכד את ה-what & why כ-spec.md, ‎/plan‎ הופך אותו לעיצוב טכני, ‎/tasks‎ מפרק אותו לעבודה — ורק אז ‎/implement‎ כותב קוד. שער אישור אנושי יושב בכל מעבר.', bullets: ['מבוסס-שלבים: Specify → Plan → Tasks → Implement', 'spec.md הוא החוזה המאושר — ה-what & why', 'שער אישור אנושי בין כל שלב', 'קוד פתוח, ניטרלי לספק (MIT)'], terminal: [ { text: '$ specify init && /specify --from PROJ-421', type: 'command' }, { text: '✓ spec.md → /plan → /tasks (awaiting approval)', type: 'success' }, { text: '⛔ /implement blocked: spec.approved = false', type: 'error' } ], repo: 'github.com/github/spec-kit', cells: ['✓ שער אכוף', '~ מוסכמות', '✗', '~ דרך סקריפטים', '✓', '✓ OSS (MIT)'] },
